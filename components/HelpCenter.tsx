...
export default function HelpCenter() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simple but highly effective email validation regex
    ...
    // Optimistic UI: Force the success state after exactly 600 milliseconds
    setTimeout(() => {
      setStatus("sent");
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] font-inter">
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 ${isOpen ? "bg-gray-100 dark:bg-[#1A1A1A] text-[#00174D] dark:text-white" : "bg-white  dark:text-black"}`}
      >
        <AnimatePresence mode="wait">
          {isOpen ? (
            <motion.svg
