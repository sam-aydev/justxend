...
export default function HelpCenter() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("open-help-center", handleTrigger);
    return () => window.removeEventListener("open-help-center", handleTrigger);
  }, []);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] font-inter">
        ...
        {isOpen && (
          ...
          >
            <div className="bg-[#00174D] p-6 text-white relative shrink-0 border-b border-gray-100">
              ...
              <p className="text-blue-100/60 text-xs mt-1 relative z-10">Typically responds in a few hours</p>
            </div>
              ...
              <AnimatePresence mode="wait">
                {status === "sent" ? (
                  ...
                  >
                    ...
                    <p className="text-gray-500 text-sm leading-relaxed">We've received your message and our team will get back to you shortly.</p>
                    <button
                  ...
                  >
                    <div className="space-y-1.5">
                      ...
                      <input name="user_name" type="text" required placeholder="John Doe" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#00174D] transition-all text-gray-900 text-sm" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest ml-1">Email Address</label>
                      <input name="user_email" type="email" required placeholder="john@example.com" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#00174D] transition-all text-gray-900 text-sm" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest ml-1">Subject</label>
                      <input name="subject" type="text" required placeholder="How can we help?" className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#00174D] transition-all text-gray-900 text-sm" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest ml-1">Message</label>
                      <textarea name="message" rows={3} required placeholder="Tell us more..." className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#00174D] transition-all text-gray-900 text-sm resize-none" />
                    </div>
                    <button
                    ...
                    >
                      {status === "sending" ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : "Send Message"}
                    </button>
                  </motion.form>
                )}
              </AnimatePresence>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 ${isOpen ? "bg-gray-100 dark:bg-[#1A1A1A] text-[#00174D] dark:text-white" : "bg-[#00174D] dark:bg-white text-white dark:text-black"}`}
      >
        <AnimatePresence mode="wait">
          {isOpen ? (
            ...
            <motion.svg key="open" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></motion.svg>
          )}
        </AnimatePresence>
      </motion.button>
